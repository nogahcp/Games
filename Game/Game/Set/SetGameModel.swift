...
struct SetGameModel {
    var stackCards: [SetCard]
    var cardOnBoard: [SetCard]
    ...
    init() {
        self.stackCards = [SetCard]()
        self.cardOnBoard = [SetCard]()
        self.selectedCards = [SetCard]()
        createStack()
        fillBoard()
    }
            ...
            for shape in propertyValues {
                for filling in propertyValues {
                    for shapeCount in propertyValues {
                        let currCard = SetCard(shape: shape, color: color, shapeCount: shapeCount, filling: filling)
    ...
    mutating private func deselectCard(currCard: SetCard) {
        self.selectedCards.remove(element: currCard)
        self.choosenCardsState = CardState.chosen
        self.score -= 1
    }

    mutating private func selectedNewCard(currCard: SetCard) {
...
//        return compareCardsValues(by: { _,_,_ in return true })
    ...
    func thirdCardForSetExist(index1: Int, index2: Int) -> Int? {
        ...
        let card3 = SetCard(shape: shape, color: color, shapeCount: shapeCount, filling: filling)
